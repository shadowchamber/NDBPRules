...
namespace xppantlr
{
    public abstract class XppLexerBase : Lexer
    {
        public XppLexerBase(ICharStream input)
            : base(input)
        {
            _input = input;
        }

        public XppLexerBase(ICharStream input, TextWriter output, TextWriter errorOutput)
        ...
        protected void OnColon()
        {
            ...
            if (interpolatedStringLevel > 0)
            {
                ...
                {
                    this.Mode(XppLexer.INTERPOLATION_FORMAT);
                }
            }
        }

parser grammar XppParser;

options { tokenVocab=XppLexer; superClass = XppParserBase; }

// entry point
// class_type? attributes?
// all_member_modifier? method_declaration?
compilation_unit
...
sql_expression
	: SELECT sql_select_options? sql_select_from_list? identifier 
		sql_orderby? 
		sql_where?
		sql_joins?
	;
...
sql_join
	: sql_join_type sql_select_from_list? identifier sql_orderby? sql_where?
	;
...
sql_select_from_list
	: sql_selecting (','  sql_selecting)* FROM
	;
...
sql_orderby
	: ORDER BY? sql_ordering (','  sql_ordering)*
	;

sql_ordering
	: sql_ordering_field dir=(ASCENDING | DESCENDING)?
	;
...
simple_embedded_statement
	...
	| WHILE sql_expression embedded_statement															  #whileSelectStatement
